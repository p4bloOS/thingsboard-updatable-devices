...
def on_message(client, userdata, msg):
    ...
    if topic_parts[2] == "LORAtoMQTT" and topic_parts[4] == "reliable":
        ...
        ack_msg = '{"message":"{' + \
            ...
            '}"}'
        print(f"Mensaje ACK a enviar: {ack_msg}")
    ...
    # Mensaje hacia LoRa (activamos rutina de reenvío hasta recibir ACK)
        ...
        print(f"Mensaje fiable para ser redirigido al dispositivo {device_id}")
    ...
    while True:
        ...
        for tries in range(MAX_RETRIES):
            ...
            if ack_received_event.wait(timeout=retry_timeout):
                ...
                print(f"Reintentando envío ({tries} intentos)")
        if not success:
...
mqttc.connect("my-iot-station.local", 1884)
